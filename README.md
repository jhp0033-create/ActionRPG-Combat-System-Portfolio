...
| 핵심 전투 | **자동 전투**, **콤보 예약 입력**, **연속 공격형 스킬**, **차지형 스킬**, **스택/차지 관리**, **Enemy FSM** |
| 구조 설계 | **FSM 상태 분리**, **ScriptableObject 데이터**, **Object Pool**, **매니저 책임 분리**, **Presenter/View 분리** |
| 조작/상태 흐름 | 좌측 터치 이동과 우측 터치 카메라 조작을 분리하고, **수동 입력·자동 이동·자동 전투 간 입력 우선순위**를 처리 |
| 플레이 완성도 | **NavMesh 기반 자동 이동** → 전투 구역 진입 → 적 스폰 → **콤보/데미지 피드백** → 클리어 팝업까지 하나의 플레이 루프로 연결 |
