...
**약 2주의 짧은 기간** 동안 모바일 액션 RPG의 **핵심 전투 흐름**을 구현한 Unity 프로토타입입니다. 자동 전투, 콤보, 스킬, 타게팅, 네비게이션, 서버 검증 모의를 하나의 플레이 흐름으로 연결했습니다.

구현의 중심은 **전투 기능을 데이터와 서비스 단위로 분리하고, 상태 전환과 입력 흐름을 명확하게 관리하는 클라이언트 구조**입니다. 짧은 기간 내 전체 RPG 시스템을 넓게 구현하기보다, 전투 로직과 그 흐름을 검증하는 데 필요한 기능을 우선했습니다.

동시에 **타격감, 발걸음, 터치 피드백, 데미지 텍스트, 클리어 프레젠테이션**처럼 전투 경험을 전달하는 시각/청각 피드백도 함께 구성했습니다. 그래픽 리소스는 무료 에셋을 기반으로 사용했으며, **파티클 조합, 머티리얼 세팅, Shader Graph 기반 UI/이펙트 표현, DOTween 연출 타이밍**은 직접 구성·튜닝했습니다.
...
| 구현 항목 | 진행 내용 | 구현 방식 |
|---|---|---|
| Auto Combat | 자동 전투 루프 구성 | 적 탐색, 접근 이동, 거리 판단, 공격 실행을 **서비스 컴포넌트**로 분리했습니다. 공격 중 이동 상태가 섞이지 않도록 흐름을 정리했습니다. |
| Attack Queue | 평타 예약 입력과 공격 타이밍 조율 | **애니메이션 진행률** 기준으로 **예약 가능 구간**을 두고, **콤보 선입력**을 버퍼링했습니다. 공격 속도 조정 후 체감 대기도 함께 보정했습니다. |
| Skill System | 서로 다른 타입의 스킬 실행 구조 | **연속 공격, 차지 공격, 스택형 공격**을 공통 실행 흐름에서 처리했습니다. 세부 판정과 연출 참조는 **스킬 데이터**와 전담 서비스로 분리했습니다. |
| Target Resolver | 동적 타게팅과 대상 전환 | **자동 전투, 수동 스킬, 적 리스트 선택** 상황에 따라 현재 타겟을 갱신하고, 공격 가능 거리와 선택 우선순위를 기준으로 대상을 전환하도록 구성했습니다. |
| Navigation System | 미니맵, 자동 이동, 자동 접근, 정지 처리 | **NavMeshAgent 기반 퀘스트 이동**과 **전투 접근**을 분리했습니다. 카메라 방향, 정지 버튼, 전투 진입 전 감속 흐름을 함께 처리했습니다. |
...
| Object Pooling | 적, VFX, UI 재사용 | 적, VFX, 데미지 텍스트를 **풀링**해 전투 중 반복 생성/파괴 비용을 줄였습니다. |
| Modular Architecture | 공통 기능 모듈과 매니저 구조 | 자동 전투, 스킬, 콤보, 타게팅, 네비게이션을 **독립 서비스 컴포넌트**로 분리했습니다. 공통 흐름은 **책임별 매니저**로 정리했습니다. |
| Data Driven | 데이터 중심 확장 구조 | 스킬, 사운드, VFX, 퀘스트, 다이얼로그, 클리어 요약 항목을 **ScriptableObject**로 관리했습니다. |
...
| Service Component | 플레이어 전투 기능 | 자동 전투, 스킬, 콤보, 타게팅, 네비게이션을 독립 책임으로 분리 |
...
미니맵은 **월드 좌표 → UI 좌표 변환**으로 플레이어/적 위치를 표시하고, 타게팅은 **자동 전투 탐색, 수동 스킬 사용, 적 리스트 선택** 상황에 맞춰 현재 대상을 동적으로 갱신하도록 구성했습니다.
...
실제 서버를 붙인 프로젝트는 아니지만, 전투 로직이 **서버 검증 구조**로 확장될 수 있도록 Mock Network와 위치 동기화 서비스를 분리해 구성했습니다.
...
Assets/_Core/Scripts
 ...
 ┣ Player       # 플레이어 이동, 전투, 스킬, 자동 전투, 네비게이션 서비스
